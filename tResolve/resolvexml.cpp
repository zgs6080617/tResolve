...
int RemoveFile()
{
	...
	struct stat st;
	char path_name[256];
	char path_full[256]; //存放需要删除文件绝对路径
...
int read_config(char* filename, config& Pa_config)
{
	...
	strncpy(sys_home_, getenv("HOME"), sizeof(sys_home_) - 1); /*获取$Home*/
	...
	for(; pElem != NULL; pElem=pElem->NextSiblingElement()) {
		...
		else if (strcmp(pattrname, "path_name") == 0) { /*不包含$HOME*/
...
int mvcim::getfilenum(char *path)
{
	...
	while((dirt = readdir(dirp)) != NULL) {
		...
		/* // @detail 为xml后缀文件并且带有"单线图"三个字(非高压用户) */
		if ((strcmp(dirt->d_name + len - 3, "xml") == 0)
			&&
			(strstr(dirt->d_name,"单线图") != NULL)) { 
...
int mvcim::mvtocim(int organ_code, Cim_Type _type)
{
	...
	if(getfilenum(trapath) == 0) {
		...
		/* // @detail 实现未筛选模型全量监测 去除数量过滤 2017-04-09 */
				...
				if (find_count_ != 0) {
					...
					for (int i = 0; i < find_count_; ++i) {
						...
						if ((itr_feeder->second.organ_code == organ_code)
							&&
							(itr_feeder->second.type == _type)) { /*名字相同并且属于同一地区并且新旧模型一致*/
...
int mvcim::transzip()
{
	...
	while((dirt = readdir(dirp)) != NULL) {
		...
		/* // @detail DMS县公司模型通过市局转发，存在测试包放置在市局服务器，第二天凌晨发送昨日测试包到考核系统，则过滤昨日包，只解析当日zip包 */
		if((strcmp(file_name+len-3,"zip") == 0)
		&&
		(strstr(file_name,monthday) != NULL) ) {

			if(strstr(file_name,"3540") != NULL ) { /*只处理正常3540压缩包*/
				startpos = strstr(file_name,"AllModel"); /*旧模型*/
				Cimendpos = strstr(file_name, "CIM"); /*新模型*/
...
#if 0
#ifndef _DMSZTGJDET
	/* // @detail 小水电明细数据没有用处，且文件较大，加载、解析占用资源，故不做解析处理 */
...
int resolvexmldata::import_xml(TiXmlDocument &doc,const char *filename,logstr& threadlog)
{
	...
	enum xmltype {
		...
		_DMSXSDET_,
		_DMSZTGJDET_,
		_DMSTRANSUP_, /*升级后(GIS1.6)配变*/
		_DMSCBUP_,	/*升级后(GIS1.6)开关*/
		_DMSDSCUP_,	/*升级后(GIS1.6)刀闸*/
		_DMSUBSUP_,	/*升级后(GIS1.6)站房*/
		_DMSBUSUP_,	/*升级后(GIS1.6)母线*/
		_DMSDEVCORR_,	/*升级前(GIS1.5)与升级后(GIS1.6)设备ID对应台账*/
		_GOMSEND_, /*GOMS推送国调考核数据记录*/
		_SMNDATA_, /*管控平台发送数据记录*/
		_SMNDATADET_, /*管控平台未发送数据错误明细*/
		_YXBS_, /*遥信辨识错误设备*/
		_END_,
		_LOG_,
		_UnKnownFileType_
	...
	for (;headElement != NULL;headElement = headElement->NextSiblingElement()) {
		...
		if (strcasecmp(headElement->Value(),"head") == 0) {
...
#if 0
						/* // @detail 取消合并，更改为拆分GPMS数据 */
						/* // @detail GPMS国网龙岩市新罗区供电公司与国网龙岩供电公司停电信息合并，
							DMS特殊处理，将国网龙岩市新罗区供电公司合并到国网龙岩供电公司*/
			...
			case _DMSYXRT_:
				{
				/* // @detail 删除原来遥信状态辨识率接入指标数据，更改为一区传输三区状态辨识错误明细数据 */
#if 0
					yxrt_vector.clear();
					yxrt yxrt_temp;
					float x = 0.0;
			...
			case _DMSDEVCORR_:
			{
				...
				for (; thrElement_corr != NULL; thrElement_corr = thrElement_corr->NextSiblingElement()) {
					...
					m_mNewId2CorrInfo.insert(make_pair(corrinfo_temp.dev_id,corrinfo_temp)); /*通过新ID找旧ID,保障新ID只有一个对应关系*/
				}
			}
				break;
			case _GOMSEND_:
...
int resolvexmldata::insertDB(int arg,logstr& threadlog,const char *filename)
{
	...
	case _DMSYXRT_:
		{
		/* // @detail 去除原来遥信状态表示率，更改为一区传入状态估计错误明细 */
#if 0
			int offsetpos = 0,number = 0;
			char query[200];
			memset(query,0x00,sizeof(query));
	...
	case _DMSDEVCORR_:
	{
		...
		if (d5000.d5000_WriteData("DMSDEVCORR", query, temp_corr, number, cyc_, attrs, error_info) != 0) {
			...
			SetGError(1, 0, g_strfiletypeName[arg], _DMSDEVCORR_, "入库成功.", threadlog);
		}
	...
	case _GOMSEND_:
	{
		...
		if (d5000.d5000_WriteData("GOMSEND", query, temp_goms, number, cyc_, attrs, error_info) != 0) {
			...
			SetGError(1, 0, g_strfiletypeName[arg], _GOMSEND_, "入库成功.", threadlog);
		}
	...
	case _SMNDATA_:
	{
		...
		if (d5000.d5000_WriteData("SMNDATA", query, temp_smndata, number, cyc_, attrs, error_info) != 0) {
			...
			SetGError(1, 0, g_strfiletypeName[arg], _SMNDATA_, "入库成功.", threadlog);
		}
	...
	case _SMNDATADET_:
	{
		...
		if (d5000.d5000_WriteData("SMNDATADET", query, temp_smndatadet, number, cyc_, attrs, error_info) != 0) {
			...
			SetGError(1, 0, g_strfiletypeName[arg], _SMNDATADET_, "入库成功.", threadlog);
		}
	...
	case _YXBS_:
	{
		...
		if (d5000.d5000_WriteData("YXBSDET", query, temp_yxbsdet, number, cyc_, attrs, error_info) != 0) {
			...
			SetGError(1, 0, g_strfiletypeName[arg], _YXBS_, "入库成功.", threadlog);
		}
...
int resolvexmldata::deleteDBDate(int code,int arg)
{
	...
	const char *g_xmltypeName[] = { /*表名*/
		...
		"EMSRT",
		//"YXRT",
		/* // @detail 去除原来遥信状态辨识了，更改为一区传入状态辨识错误明细 */
		"DMSZTGJCBDET",
		"DMSLINE",
		"DMS_XSD_DET",
		"DMS_ZTGJ_DET",
	...
	case _DMSXSD_:
		...
		break;
	case _DMSYXRT_:
		/* // @detail 状态估计文件为当日实时发送一区，所以count_time为前日时间 */
