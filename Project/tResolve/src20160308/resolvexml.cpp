...
int resolvexmldata::import_xml(TiXmlDocument &doc,const char *filename,logstr& threadlog)
{
	...
	enum xmltype {_ZZZX_,_DATT_,_DAGC_,_YXBW_,_GPZW_,_DMSGPZW_,_GPMSGPZW_,_ZDZX_,_YKCZ_,_YXINFO_,_RATE_,_BUS_,_CB_,_DSC_,_TRANS_,_SUBS_,_TDTRANS_,_AUTOMAP_,_TQREALEASE_,_FASTR_,_TZLIST_,_GPMSTD_,_GPMSTDDET_,_GPMSZLPDET_,_DMSZLPDET_,_DMSXSD_,_ZTGJ_,_END_,_LOG_,_UnKnownFileType_}g_xmltype;

	const char *g_xmltypeName[] = {"ZZZX","DATT","DAGC","YXBW","GPZW","DMSGPZW","GPMSGPZW","ZDZX","YKCZ","YXINFO","dmsindex","buslist","cblist","dsclist","translist","subslist","tdtrans","automap","tdrelease","fastr","tzlist","gpmstd","gpmstddet","gpmszlpdet","zlp","wkwh","ztgj","END","LOG"};
...
int resolvexmldata::insertDB(int arg,logstr& threadlog,const char *filename)
{
	...
	enum filetype {_ZZZX_,_DATT_,_DAGC_,_YXBW_,_GPZW_,_DMSGPZW_,_GPMSGPZW_,_ZDZX_,_YKCZ_,_YXINFO_,_RATE_,_BUS_,_CB_,_DSC_,_TRANS_,_SUBS_,_TDTRANS_,_AUTOMAP_,_TQRELEASE_,_FASTR_,_TZLIST_,_GPMSTD_,_GPMSTDDET_,_GPMSZLPDET_,_DMSZLPDET_,_DMSXSD_,_ZTGJ_,_END_,_UnKnownFileType_}g_filetype;
	const char *g_strfiletypeName[] = {"DMSOP","DATT","DAGC","YXBW","GPZW","DMSGPZW","GPMSGPZW","ZDZX","YKCZ","YXINFO","YCRATE","DMSBUS","DMSCB","DMSCB","DMSLD","DMSST","DMSTD","AUTOGRAPH","TDINFO","FACTION","TZDETAILS","GPMSTD","GPMSTDDET","GPMS_ZLP","DMS_ZLP","DMS_XSD","DMS_ZTGJ","END"};
...
int resolvexmldata::deleteDBDate(int code,int arg)
{
	...
	//enum filetype {_ZZZX_,_DATT_,_DAGC_,_YXBW_,_GPZW_,_DMSGPZW_,_GPMSGPZW_,_ZDZX_,_YKCZ_,_YXINFO_,_DMSCIME_,_GPMSCIME_,_DMSDATA_,_GPMSDATA_,_END_,_UnKnownFileType_}g_filetype;
	enum filetype {_ZZZX_,_DATT_,_DAGC_,_YXBW_,_GPZW_,_DMSGPZW_,_GPMSGPZW_,_ZDZX_,_YKCZ_,_YXINFO_,_RATE_,_BUS_,_CB_,_DSC_,_TRANS_,_SUBS_,_TDTRANS_,_AUTOMAP_,_TQRELEASE_,_FASTR_,_TZLIST_,_GPMSTD_,_GPMSTDDET_,_GPMSZLPDET_,_DMSZLPDET_,_DMSXSD_,DMS_ZTGJ,_END_,_UnKnownFileType_}g_filetype;

	//const char *g_xmltypeName[] = {"DMSOP","DATT","DAGC","YXBW","GPZW","DMSGPZW","GPMSGPZW","ZDZX","YKCZ","YXINFO","DMSCIME","GPMSCIME","DMSDATA","GPMSDATA","END"};
	const char *g_xmltypeName[] = {"DMSOP","DATT","DAGC","YXBW","GPZW","DMSBRAND","GPMSGPZW","ZDZX","YKCZ","YXINFO","YCRATE","DMSBUS","DMSCB","DMSCB","DMSLD","DMSST","DMSTD","AUTOGRAPH","TDINFO","FACTION","TZDETAILS","GPMSTD","GPMSTDDET","GPMS_ZLP","DMS_ZLP","DMS_XSD","DMS_ZTGJ","END"};
...
//根据ctl_name判断指令票所属公司类型
int check_ctl_type(const char * ctl_name)
{
	if(ctl_name == NULL)
		return -1;
	char dc_str[][4] = {"FZ","XM","ND","PT","QZ","ZZ","LY","SM","NP"};
	char tmp_ctlname[20];
	int dc_flag = 0;
	memset(tmp_ctlname,0x00,sizeof(tmp_ctlname));
	strncpy(tmp_ctlname,ctl_name,sizeof(tmp_ctlname));
	printf("tmp_ctlname:%s\n",tmp_ctlname);
	const char* split = "_";
	char * e_dvid;
    e_dvid=strtok(tmp_ctlname,split);
	char m_name[2][10];
	for(int i = 0;i < ZLP_CTL_SIZE;i++)
	{
		e_dvid=strtok(NULL,split);
		strncpy(m_name[i],e_dvid,sizeof(m_name[i]));
		printf("m_name[%d]:%s\n",i,m_name[i]);
	}
	
	for(int i=0;i < DC_SIZE;i++)
	{
		if(strcmp(dc_str[i],m_name[0]) == 0)
		{
			dc_flag = 1;
			break;
		}
	}
	if(dc_flag == 1 && strcmp(m_name[0],m_name[1]) == 0)
	{
		return 1;
	}
	else
