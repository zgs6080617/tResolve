	...
	case _TQRELEASE_:
		{
			...
			for(it_tqrelease = tqrelease_vector.begin();it_tqrelease != tqrelease_vector.end();++it_tqrelease)
			{
				...
				offsetpos += STR_LEN_SS;*/
				sprintf(query,"INSERT INTO EVALUSYSTEM.DETAIL.TDINFO(ORGAN_CODE,TDNUM,DMSTD,DDAFF,DMSFF,TDINFO,COUNT_TIME,SEND_TIME,SENDTYPE) VALUES(%d,%d,to_date('%s','YYYY-MM-DD HH24:MI:SS'),to_date('%s','YYYY-MM-DD HH24:MI:SS'),to_date('%s','YYYY-MM-DD HH24:MI:SS'),'%s',to_date('%s','YYYY-MM-DD HH24:MI:SS'),to_date('%s','YYYY-MM-DD HH24:MI:SS'),'%s')",
					it_tqrelease->header.code,it_tqrelease->tdnum,it_tqrelease->dmstd,it_tqrelease->ddaff,it_tqrelease->dmsaff,it_tqrelease->tdintnr,it_tqrelease->header.count,it_tqrelease->header.send,it_tqrelease->sendtype);
	...
	case _GPMSTDDET_:
		{
			...
			struct ColAttr* attrs = (ColAttr* )malloc(sizeof(ColAttr_t)*13);
			if(attrs == NULL)
				break;
			attrs[0].data_type = DCI_INT;
			attrs[0].data_size = INT_LEN;
			...
			attrs[11].data_size = STR_LEN_DETAILS;
			...
			for(it_gpmstddet = gpmstddet_vector.begin();it_gpmstddet != gpmstddet_vector.end();++it_gpmstddet)
			{
				...
				offsetpos += STR_LEN_CHAR;
				memcpy(temp_gpmstddet+offsetpos,it_gpmstddet->details,STR_LEN_DETAILS);
				offsetpos += STR_LEN_DETAILS;
				memcpy(temp_gpmstddet+offsetpos,it_gpmstddet->tdintr,STR_LEN_DETAILS);
				offsetpos += STR_LEN_DETAILS;
			}
			strcpy(query,"INSERT INTO EVALUSYSTEM.DETAIL.GPMS_POWERCUT_DET(ORGAN_CODE,ORGAN_NAME,DMSID,GPMSID,FAULT_TIME,RETURN_TIME,FAULT_LINE,COUNT_TIME,SEND_TIME,FAULT_NO,BILLS_NUM,DETAILS,TDINTR) VALUES(:1,:2,:3,:4,to_date(:5,'YYYY-MM-DD HH24:MI:SS'),to_date(:6,'YYYY-MM-DD HH24:MI:SS'),:7,to_date(:8,'YYYY-MM-DD HH24:MI:SS'),to_date(:9,'YYYY-MM-DD HH24:MI:SS'),:10,:11,:12,:13)");
			if(d5000.d5000_WriteData("GPMS_POWERCUT_DET",query,temp_gpmstddet,number,13,attrs,error_info) != 0)
			{
