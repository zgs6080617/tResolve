...
int resolvexmldata::import_xml(TiXmlDocument &doc,const char *filename,logstr& threadlog)
{
	...
	enum xmltype {_ZZZX_,_DATT_,_DAGC_,_YXBW_,_GPZW_,_DMSGPZW_,_GPMSGPZW_,_ZDZX_,_YKCZ_,_YXINFO_,_RATE_,_BUS_,_CB_,_DSC_,_TRANS_,_SUBS_,_TDTRANS_,_AUTOMAP_,_TQREALEASE_,_FASTR_,_TZLIST_,_GPMSTD_,_GPMSTDDET_,_GPMSZLPDET_,_DMSZLPDET_,_DMSXSD_,_END_,_LOG_,_UnKnownFileType_}g_xmltype;

	const char *g_xmltypeName[] = {"ZZZX","DATT","DAGC","YXBW","GPZW","DMSGPZW","GPMSGPZW","ZDZX","YKCZ","YXINFO","dmsindex","buslist","cblist","dsclist","translist","subslist","tdtrans","automap","tdrelease","fastr","tzlist","gpmstd","gpmstddet","gpmszlpdet","zlp","wkwh","END","LOG"};
				...
				case _DMSXSD_:
				{
					...
					for (;thrElement_gpmstddet != NULL;thrElement_gpmstddet = thrElement_gpmstddet->NextSiblingElement())
					{
						...
						for(;attributeOfthr_gpmstddet != NULL;attributeOfthr_gpmstddet = attributeOfthr_gpmstddet->Next())
						{
							...
							if(strcasecmp(attname,"wkwhsum") == 0)
								temp_dmsxsd.value=atof(attvalue);
 							else
								cout << "Unknown attribute:" << attvalue << endl;
						}
...
int resolvexmldata::insertDB(int arg,logstr& threadlog,const char *filename)
{
	...
	enum filetype {_ZZZX_,_DATT_,_DAGC_,_YXBW_,_GPZW_,_DMSGPZW_,_GPMSGPZW_,_ZDZX_,_YKCZ_,_YXINFO_,_RATE_,_BUS_,_CB_,_DSC_,_TRANS_,_SUBS_,_TDTRANS_,_AUTOMAP_,_TQRELEASE_,_FASTR_,_TZLIST_,_GPMSTD_,_GPMSTDDET_,_GPMSZLPDET_,_DMSZLPDET_,_DMSXSD_,_END_,_UnKnownFileType_}g_filetype;
	const char *g_strfiletypeName[] = {"DMSOP","DATT","DAGC","YXBW","GPZW","DMSGPZW","GPMSGPZW","ZDZX","YKCZ","YXINFO","YCRATE","DMSBUS","DMSCB","DMSCB","DMSLD","DMSST","DMSTD","AUTOGRAPH","TDINFO","FACTION","TZDETAILS","GPMSTD","GPMSTDDET","GPMS_ZLP","DMS_ZLP","DMS_XSD","END"};
	...
	case _GPMSZLPDET_:
		{
			...
			struct ColAttr* attrs = (ColAttr* )malloc(sizeof(ColAttr_t)*11);
			if(attrs == NULL)
				break;
			attrs[0].data_type = DCI_INT;
			attrs[0].data_size = INT_LEN;
			...
			strcpy(query,"INSERT INTO EVALUSYSTEM.DETAIL.GPMS_ZLP(ORGAN_CODE,ORGAN_NAME,DMSID,GPMSID,TYPE,CONTENT,START_TIME,ORDER_TIME,CONFIRM_TIME,COUNT_TIME,SEND_TIME) VALUES(:1,:2,:3,:4,:5,:6,to_date(:7,'YYYY-MM-DD HH24:MI:SS'),to_date(:8,'YYYY-MM-DD HH24:MI:SS'),to_date(:9,'YYYY-MM-DD HH24:MI:SS'),to_date(:10,'YYYY-MM-DD HH24:MI:SS'),to_date(:11,'YYYY-MM-DD HH24:MI:SS'));");
		
			if(d5000.d5000_WriteData("GPMS_ZLP",query,temp_gpmszlpdet,number,11,attrs,error_info) != 0)
			{
	...
	case _DMSXSD_:
		{
			...
			sprintf(sql,"DELETE FROM DETAIL.DMS_XSD WHERE organ_code = %d and COUNT_TIME = to_date(sysdate-1,'YYYY-MM-DD')",threadlog.code);
			
			if(d5000.DeleteDBDate(sql) != 0)
			{
				Log("delete DMS_XSD fail.\n");
				ret = ret | errmask;
			}
			else
			{
				Log("delete DMS_XSD success.\n");
			}
			printf("%s\n",sql);
			...
			if(d5000.d5000_WriteData("DMS_ZLP",query,temp_dmsxsd,number,4,attrs,error_info) != 0)
			{
...
int resolvexmldata::deleteDBDate(int code,int arg)
{
	...
	//const char *g_xmltypeName[] = {"DMSOP","DATT","DAGC","YXBW","GPZW","DMSGPZW","GPMSGPZW","ZDZX","YKCZ","YXINFO","DMSCIME","GPMSCIME","DMSDATA","GPMSDATA","END"};
	const char *g_xmltypeName[] = {"DMSOP","DATT","DAGC","YXBW","GPZW","DMSGPZW","GPMSGPZW","ZDZX","YKCZ","YXINFO","YCRATE","DMSBUS","DMSCB","DMSCB","DMSLD","DMSST","DMSTD","AUTOGRAPH","TDINFO","FACTION","TZDETAILS","GPMSTD","GPMSTDDET","GPMS_ZLP","DMS_ZLP","DMS_XSD","END"};
