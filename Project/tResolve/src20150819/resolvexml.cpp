...
//删除dmsxml数据文件，深夜删除当天dmsxml文件，以免干扰凌晨指标计算，add by lcm 20150727
int RemoveFile()
{
	DIR *dirp;
	struct dirent *dirt;
...
int TruncateHisrecord()
{
	...
	if(month_num % 2 != 0 && tm_temp->tm_wday == 1)	//判断是否单周周一
	{
		...
		while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
		{
			data_flag = d5000.d5000_ExecSingle(qury);
			if(data_flag != 0)
			{
...
void resolvexmldata::GetGError(logstr& threadlog)
{
	...
	const char *g_indexname[] = {"配电主站月平均运行率","FA投运率","遥信正确率","挂牌信息一致率","拓扑正确率","终端在线率","遥控成功率","遥控使用率",
		...
		"DMS停电信息发布完整率","自动成图率","自动成图数量","停电信息确认率","停电信息发布及时率","停电信息发布时间","FA动作次数","公专变明细","停电故障率","停电故障明细"};
	...
	while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
	{
		data_flag = d5000.d5000_ExecSingle(temp);
		if(data_flag != 0)
		{
			d5000.d5000_DisConnect();
			if(d5000.d5000_Connect() != 0)
...
void *process_traversal()
{
	...
	if(sign == 0)
	{
		...
		while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
		{
			data_flag = d5000.d5000_WriteData("DATAREC",query,tmp_datarec,number,4,attrs,error_info);
			if(data_flag !=0)
			{
				d5000.d5000_DisConnect();
				d5000.d5000_Connect();
			...
			case _CB_:
				{
					...
					for (;thrElement_cb != NULL;thrElement_cb = thrElement_cb->NextSiblingElement())
					{
						...
						if(strcasecmp(cb_temp.vol,"0.38") != 0)
						{
							...
							//printf("cb:%s,status:%s\n",cb_temp.cbid,cb_temp.status);
						}
					}
				}
				break;
					...
					for (;thrElement_tqrelease != NULL;thrElement_tqrelease = thrElement_tqrelease->NextSiblingElement())
					{
						...
						for(;attributeOfthr_tqrelease != NULL;attributeOfthr_tqrelease = attributeOfthr_tqrelease->Next())
						{
							...
							else if(strcasecmp(attname,"type") == 0)
							{
								...
								//strncpy(tqrelease_temp.sendtype,attvalue,sizeof(tqrelease_temp.sendtype));
							else
								cout << "Unknown attribute:" << attvalue << endl;
						}
	...
	case _ZZZX_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("DMSOP",query,temp_dmsop,number,4,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_ZZZX_,error_info,threadlog);
					Log("Insert DMSOP fail.\n");
	...
	case _DATT_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("DATT",query,temp_datt,number,6,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_DATT_,error_info,threadlog);
					Log("Insert DATT fail.\n");
	...
	case _YXBW_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("YXBW",query,temp_yxbw,number,6,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_YXBW_-1,error_info,threadlog);
					Log("Insert YXBW fail.\n");
	...
	case _DMSGPZW_:
		{
			...
			if(dmsbrand_vector.size() != 0)
			{
				...
				while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
				{
					data_flag = d5000.d5000_WriteData("DMSBRAND",query,temp_dmsbrand,number,6,attrs,error_info);
					if(data_flag !=0)
					{
						SetGError(2,0,g_strfiletypeName[arg],_DMSGPZW_-2,error_info,threadlog);
						Log("Insert DMSBRAND fail.\n");
	...
	case _ZDZX_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("ZDZX",query,temp_zdzx,number,6,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_ZDZX_-2,error_info,threadlog);
					Log("Insert ZDZX fail.\n");
	...
	case _YKCZ_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("YKCZ",query,temp_ykcz,number,6,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_YKCZ_-2,error_info,threadlog);
					Log("Insert YKCZ fail.\n");
	...
	case _YXINFO_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("YXINFO",query,temp_yxinfo,number,6,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_YXINFO_-2,error_info,threadlog);
					Log("Insert YXINFO fail.\n");
	...
	case _RATE_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("YCRATE",query,temp_rate,number,4,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_RATE_-2,error_info,threadlog);
					Log("Insert Rate fail.\n");
	...
	case _BUS_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("DMSBUS_IMAGE",query,temp_bus,number,5,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_BUS_-1,error_info,threadlog);
					Log("Insert bus fail.\n");
	...
	case _CB_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("DMSCBL_IMAGE",query,temp_cb,number,8,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_CB_-1,error_info,threadlog);
					Log("Insert cb fail.\n");
	...
	case _DSC_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("DMSCBL",query,temp_dsc,number,8,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_DSC_-1,error_info,threadlog);
					Log("Insert cb fail.\n");
	...
	case _TRANS_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("DMSLDL",query,temp_trans,number,5,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_TRANS_,error_info,threadlog);
					Log("Insert ld fail.\n");
	...
	case _SUBS_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("DMSST",query,temp_subs,number,5,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_SUBS_,error_info,threadlog);
					Log("Insert st fail.\n");
	...
	case _TDTRANS_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("DMSTD",query,temp_tdtrans,number,6,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_TDTRANS_,error_info,threadlog);
					Log("Insert dmstd fail.\n");
	...
	case _AUTOMAP_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("AUTOGRAPH",query,temp_automap,number,5,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_AUTOMAP_,error_info,threadlog);
					Log("Insert automap fail.\n");
	...
	case _TQRELEASE_:
		{
			...
			for(it_tqrelease = tqrelease_vector.begin();it_tqrelease != tqrelease_vector.end();++it_tqrelease)
			{
				...
				offsetpos += STR_LEN_SS;*/
				sprintf(query,"INSERT INTO EVALUSYSTEM.DETAIL.TDINFO(ORGAN_CODE,TDNUM,DMSTD,DDAFF,DMSFF,TDINTR,COUNT_TIME,SEND_TIME,SENDTYPE,JUDGED_TIME) VALUES(%d,%d,to_date('%s','YYYY-MM-DD HH24:MI:SS'),to_date('%s','YYYY-MM-DD HH24:MI:SS'),to_date('%s','YYYY-MM-DD HH24:MI:SS'),'%s',to_date('%s','YYYY-MM-DD HH24:MI:SS'),to_date('%s','YYYY-MM-DD HH24:MI:SS'),%d,to_date('%s','YYYY-MM-DD HH24:MI:SS'))",it_tqrelease->header.code,it_tqrelease->tdnum,it_tqrelease->dmstd,it_tqrelease->ddaff,it_tqrelease->dmsaff,it_tqrelease->tdintnr,it_tqrelease->header.count,it_tqrelease->header.send,it_tqrelease->sendtype,it_tqrelease->judged_time);
				...
				while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
				{
					data_flag = d5000.d5000_ExecSingle(query);
					if(data_flag != 0)
					{
						Log("Insert tdinfo fail.\n");
						ret = ret | errmask;
	...
	case _FASTR_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("FACTION",query,temp_fastr,number,6,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_FASTR_+3,error_info,threadlog);
					Log("Insert fades fail.\n");
	...
	case _TZLIST_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("TZDETAILS",query,temp_tz,number,7,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_TZLIST_+3,error_info,threadlog);
					Log("Insert fades fail.\n");
	...
	case _GPMSTD_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("GPMS_POWERCUT",query,temp_gpmstd,number,5,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_GPMSTD_+3,error_info,threadlog);
					Log("Insert fades fail.\n");
	...
	case _GPMSTDDET_:
		{
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.d5000_WriteData("GPMS_POWERCUT_DET",query,temp_gpmstddet,number,16,attrs,error_info);
				if(data_flag !=0)
				{
					SetGError(2,0,g_strfiletypeName[arg],_GPMSTDDET_+3,error_info,threadlog);
					Log("Insert GPMS_POWERCUT_DET fail.\n");
...
int resolvexmldata::insertDBLOG(int arg,int code,int retcode,const char *filename)
{
	...
	while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
	{
		data_flag = d5000.d5000_WriteData("LOG",query,temp,1,5,attrs);
		if(data_flag !=0)
		{
			d5000.d5000_DisConnect();
			d5000.d5000_Connect();
...
int resolvexmldata::deleteDBDate(int code,int arg)
{
	...
	if(del == 1)
	{
		...
		case _YXINFO_:
			...
			while(data_count < 3)//处理数据库断开问题 add by lcm 20150818
			{
				data_flag = d5000.DeleteDBDate(sql);
				if(data_flag !=0)
				{
					d5000.d5000_DisConnect();
					d5000.d5000_Connect();
					data_flag = data_flag | mask;
					data_count++;
				}
				else 
				{
